...
class TestIcuRecipe(unittest.TestCase):
    ...
    def test_should_build(self, mock_makedirs):
    ...
    @mock.patch("pythonforandroid.recipes.icu.ensure_dir")
    @mock.patch("pythonforandroid.recipes.icu.sh.cp")
    @mock.patch("pythonforandroid.recipes.icu.glob")
    @mock.patch("pythonforandroid.util.makedirs")
    def test_copy_files(
        self, mock_makedirs, mock_glob, mock_sh_cp, mock_ensure_dir
    ):
        arch = ArchARMv7_a(self.ctx)
        recipe = ICURecipe()
        recipe.ctx = self.ctx
        mock_glob.return_value = ["/fake_dir/libicu57.so"]
        recipe.copy_files(arch)
        mock_makedirs.assert_called()
